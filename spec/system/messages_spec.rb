...
RSpec.describe 'メッセージ投稿機能', type: :system do
  ...
  context '投稿に成功したとき' do
    ...
    it 'チャットルームを削除すると、メッセージもDBから削除される' do
      # サインインする
      sign_in(@room_user.user)
      # 作成されたチャットルームへ遷移する
      click_on(@room_user.room.name)
      #メッセージをDBへ保存する
      FactoryBot.create_list(:message, 3,room_id: @room_user.room.id, user_id: @room_user.user.id)
      #チャットルームを削除する
      #DBに当該メッセージが残っていないことを確かめる
      expect {
        find_link('チャットを削除する',  href: room_path(@room_user.room)).click
      }.to change { @room_user.room.messages.count }.by(-3)
      #トップページに戻っていることを確かめる
